...
- **agent channel 推送（hermes agent gateway）**：推送动作由 **agent 执行**——
  ...
  钉钉/企业微信等频道凭据；**hermes v0.20+ 强制显式 `--to`，目标取项目配置
  `publish_defaults.hermes_send_targets` 列表**，如 `[weixin, telegram:12345]`；
  **空 = 推送到所有已发现 channel**）。无需额外配置文件；目标枚举失败、
  hermes 未安装或 gateway 未运行时仅警告、不影响流程，agent
