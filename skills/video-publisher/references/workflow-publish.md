...
成功特征）表示脚本无法自动确认提交结果，**无需人工确认页面状态**——重试是
安全的：框架 run() 的幂等保护会在重跑时自动检测页面是否已是成功状态
（`check_already_submitted` 命中则直接跳过、不重复上传/提交）。agent 收到
失败可直接重试。发布成功但无封面时脚本输出 `cover_missing` 提示，需先解决
封面（补 workflow 配置或用户提供封面）再处理，不要带着缺封面继续发多条。
